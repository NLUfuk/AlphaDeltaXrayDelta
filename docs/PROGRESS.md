...
### Faz 38 — JWT anahtar ayrımı + doküman yeri ✅ (2026-08-06)

- [x] **JWT imza anahtarı lokal ile canlıda aynıydı — ayrıldı.** Aynı anahtar demek, lokalde üretilen
  bir access/refresh token'ın canlıda da geçerli olması demekti; geliştirme makinesindeki `.env`
  sızsa canlıda istenen kullanıcı adına token üretilebilirdi. Canlıya 48 baytlık yeni bir anahtar
  (`appsettings.Production.json` → app pool restart), lokale **ayrı** bir anahtar yazıldı.
  Doğrulandı: eski anahtarla imzalı token canlıda **401**, yeni anahtarla login **200**.
  Yan etki (beklenen): mevcut tüm oturumlar düştü, herkes yeniden giriş yapacak.
- [x] **Kök dizindeki `.md` dosyaları `docs/` altına alındı** (kullanıcı isteği: GitHub'da README
  dışında md görünmesin). `git mv` ile taşındı, yani **dosya geçmişi korundu**; README ve `publish.ps1`
  içindeki yollar güncellendi.

**Karar/Varsayım (Faz 38):**
- *Dokümanlar silinmedi/repodan çıkarılmadı, taşındı.* İki alternatif vardı: `git rm --cached` +
  gitignore (GitHub'da hiç görünmez) ya da ayrı branch. Kullanıcıya üçü de sunuldu, `docs/` seçildi.
  Gerekçe kayda değer: `PROGRESS.md` 173 KB'lik karar günlüğü; repodan çıkarmak onu versiyonsuz ve
  yedeksiz bırakırdı — makine giderse kayıt da giderdi. Kök temizliği bu bedeli gerektirmiyordu.
- *CLAUDE.md "proje kökünde tek bir PROGRESS.md" diyor; dosya artık `docs/PROGRESS.md`.* Kural, kaydın
  **tek ve bulunabilir** olmasını amaçlıyor — iki kopya olmasın diye. Tek kopya ve README'den
  bağlantılı olduğu sürece amaç korunuyor. Yeni oturumda önce `docs/PROGRESS.md` okunur.
- *Lokal anahtar da yenilendi, sadece canlı değil.* Eski anahtar bu oturumun araç çıktılarında görünmüştü;
  ikisini de yenilemek tek satırlık ek işti, lokal oturumların düşmesinin maliyeti yok.

### Faz 39 — Müşteri ↔ şirket para ilişkisi: talep tutarı + Kazanç raporu ✅ (2026-08-06)

Pano zaten bir **fırsat hattı** gibi kullanılıyordu (Faz 23'ten beri Odoo görünümü, "fırsat havuzu"),
ama hiçbir talebin parasal karşılığı yoktu: "bu müşteri bize ne kazandırdı" sorusunun cevabı sistemde
yoktu. Bu faz o ilişkiyi kurar.

- [x] **`Ticket.EstimatedValue` + `ActualValue`** (decimal(18,2), nullable) + `SetValue()` + türetilmiş
  `ReportableValue` (gerçekleşen varsa o, yoksa tahmin). Migration `20260806132314_TicketValue`.
- [x] **Yeni yetki `ticket.value`** — `PermissionKeys` + Türkçe etiket/açıklama + rol matrisi
  (Admin, SuperAdmin varsayılan; Personel'de kapalı). `POST /api/tickets/{id}/value` bu yetkiyi ister.
- [x] **Okuma yollarının hepsinde sunucu tarafı kısıtlama** — talep detayı (`estimatedValue`/`actualValue`
  + `canSeeValue`), kanban kartı, liste, rapor. Yetki yoksa alan **null gelir**, gizlenmez.
- [x] **`RevenueSummary`** rapora eklendi: kazanılan/kaybedilen/açık hat (tutar + adet), tutarı
  girilmemiş talep sayısı, kazanma oranı (adetçe ve tutarca ayrı), tahmin isabeti, aylık trend.
- [x] **`finance.currency` ayarı** (varsayılan `TRY`) seed'e eklendi; para birimi rakamlarla birlikte
  DTO'da taşınıyor (`/settings` yalnız süper admine açık, birimsiz rakam rakam değildir).
- [x] **Frontend:** `Kazanç` sekmesi (`/revenue`) — 4 kutu + aylık kazanılan/kaybedilen grafiği
  (`RevenueChart`) + tutar dağılımı; talep detayında tahmini/gerçekleşen tutar kartı; kanban kartında
  kompakt tutar rozeti (145B / 1,2M).
- [x] **Denetim izi:** tutar değişikliği `TicketEventType.ValueChanged` olarak `TicketEvents`'e yazılıyor
  (eski→yeni "tahmini/gerçekleşen" biçiminde).
- [x] **19 yeni test** (`RevenueReportTests` 9, `TicketValueVisibilityTests` 10) — toplam **233 yeşil**.

**Karar/Varsayım (Faz 39):**
- *Kazanıldı/kaybedildi statü **kategorisinden** okunuyor, ayrı alan yok.* Spec bir "satış sonucu" alanı
  tanımlamıyor. İkinci bir alan, biri talebi yeniden açtığında ya da farklı bir statüye kapattığında
  anında ikinci bir gerçek olurdu. Kategori üzerinden okumak, şirketin "Tamamlandı"yı "Teslim edildi"
  diye yeniden adlandırmasına da dayanıklı (spec §4.3 şirket-özel statüler).
- *Tahmin ve gerçekleşen **iki ayrı alan**.* Tek alanla, kapanışta tahmin üzerine yazılır ve "tahminlerim
  ne kadar tutuyor" sorusu cevapsız kalırdı — Kazanç sekmesinin varlık sebebinin yarısı bu.
  Kazanılmış talepte gerçekleşen boşsa tahmin geçerli sayılır; aynı rakam iki kez yazılmaz.
- *`ticket.value` ayrı bir yetki, `ticket.view`/`ticket.edit` içine katlanmadı.* Ticari hacim, talebin
  kendisinden **farklı türde** bir sır: birini şirkete eklemek ona sipariş defterini vermemeli, talepteki
  yazım hatasını düzelten kişi de otomatik olarak fiyat belirleyen kişi değildir.
- *Yetkisiz kullanıcıdan rakam **çıkarılıyor**, gizlenmiyor.* Gizlemek UI kararıdır; devtools açan
  görürdü. Bu yüzden dört okuma yolunun her biri ayrı ayrı test edildi, tek ortak yardımcıya güvenilmedi.
- *Negatif tutar reddediliyor (domain kuralı).* Kaybedilen iş, "negatif tutar" değil, **LOST sonuçlu**
  tutardır. İkisine birden izin vermek aynı kaybı iki kez düşürürdü (bir kez işaretle, bir kez sınıfla).
- *Boş tutar ≠ 0.* Fiyatlanmamış talep hiçbir toplama katılmaz, ayrı sayılır (`unpricedCount`).
  Sıfır saymak tahminleri sistematik olarak aşağı çekerdi.
- *Tek para birimi (ayar), çoklu değil.* Çoklu para birimi geçmiş kur tablosu ister; bugünün kuruyla
  çevirmek geçen yılın cirosunu sessizce yeniden yazardı. İkinci gerçek ihtiyaç çıkana kadar tek birim.
- *Kazanma oranı 0/0 iken `null`, 0 değil.* Hiçbir şey kapanmamış şirkete "%0 kazanma oranı" yazmak
  yanlış bilgi; ekranda `—` görünür.
- *Tutar değişikliği denetime yazılıyor.* Sınıftaki diğer tüm mutasyonlar `TicketEvent` bırakıyordu;
  para, "kim ne zaman neye çekti" diye sonradan sorulan tek alan. Tutarsızlık bu fazda kapatıldı.

**Açık iş (Faz 39 sonrası):**
- **Canlıya alınmadı.** `TicketValue` migration'ı + yeni yetkinin seed'i canlıda çalışmalı (API açılışta
  migrate+seed ediyor). Deploy elle (CI'da adım yok) — `publish.ps1` + SHA256 dosya farkı yöntemi.
- CSV export'ta tutar sütunu **yok** (`ExportRow` dokunulmadı). Eklenirse `ticket.value` yetkisine göre
  sütunun tamamı çıkarılmalı — rapor DTO'sundaki kuralın aynısı. Kapsam dışı bırakıldı, teknik borç #43.
- Lokal dev veritabanında `CustomerIntakeTrust` ve `TicketValue` migration'ları **pending** (API bu
  oturumda hiç açılmadı); ilk açılışta uygulanacak. Canlıda `CustomerIntakeTrust` zaten var (Faz 37).

### Faz 40 — Pano "+Yeni" regresyonu + Kanby markası ✅ (2026-08-06)

- [x] **Pano başlığındaki "+Yeni" butonu hiçbir şey yapmıyordu — kök neden bulundu ve kapatıldı.**
  Buton yalnız `composeIn`'i set ediyordu; ama Faz 36'da kutular açılır/kapanır hale gelince
  hızlı-ekleme formu **yalnız açık kutunun içinde** render edilmeye başladı. Kutu kapalıyken form
  hiç basılmıyordu, yani tıklama sessizce kayboluyordu. Sütun içindeki `+` butonu doğru davranıyordu
  (`setExpanded` + `setComposeIn`) — yani hata tek bir çağrı yerinde değil, **iki giriş noktasının
  ayrışmasındaydı**. İkisi de tek `openCompose(statusId)` fonksiyonuna bağlandı; sütunlar yüklenmeden
  buton `disabled`.
- [x] **Marka: CRM Kanban → Kanby.** `kanby-logo.svg` (kullanıcı verdi) `frontend/public/` altına
  alındı; işaret + kelime markası + slogan `ui/Logo.tsx` olarak yeniden çizildi ve **sidebar + giriş
  ekranında** kullanılıyor. Sekme başlığı `Kanby — Connect · Convert`, favicon düğüm grafiği oldu.
  `brand.system_name` seed varsayılanı `Kanby`, `brand.logo_url` → `/kanby-logo.svg`.
- [x] **Para alanlarının hassasiyeti açıkça bildirildi** (`HasPrecision(18,2)`) — lokal çalıştırmada
  EF "no store type specified for decimal … values will be silently truncated" uyarısı verdi.
  Şema değişmiyor (EF varsayılanı zaten 18,2) → **migration yok**, `has-pending-model-changes` temiz.
- [x] **Lokal uçtan uca doğrulama** (gerçek SQL Server + API + SPA, tarayıcıdan):
  bekleyen iki migration uygulandı (`CustomerIntakeTrust`, `TicketValue`) → giriş 200 → "+Yeni" ile
  **TEKSTIL-14 oluşturuldu (201)** → tutar kaydedildi (`POST /tickets/{id}/value` → **204**) →
  kanban kartında `145B` rozeti → Kazanç ekranında `₺145.000` açık hat, kazanma oranı `—`
  (hiçbir şey kapanmadığı için `null`, %0 değil).

**Karar/Varsayım (Faz 40):**
- *Logo `<img>` olarak değil, inline SVG bileşeni olarak.* İki gerçek sebep: `<img>` koyu temayı
  takip edemez (kaynak lacivert #0F2540, koyu zeminde görünmez) ve daraltılmış sidebar yalnız
  işareti ister. Çizgiler `currentColor`, **yalnız amber düğüm** literal kaldı — logoyu logo yapan
  tek nokta o, iki temada da aynı kalmalı. Orijinal dosya `public/kanby-logo.svg` olarak duruyor
  (dışa aktarım ve `brand.logo_url` için).
- *Favicon'da `currentColor` kullanılmadı, sabit lacivert yazıldı.* Favicon'u tarayıcı çerçevesi
  çiziyor; sayfa temasını da rengi de devretmez.
- *Sidebar'daki marka adı hâlâ kodda sabit, `brand.system_name`'den okunmuyor.* Ayar zaten öyleydi
  (eskiden de "CRM Kanban" sabitti). Ayardan okumak için `/settings` ucunun personele açılması
  gerekir — bu bir yetki genişlemesi, marka değişikliğinin bedeli olamaz. Teknik borç #45.
- *`brand.system_name`/`brand.logo_url` canlıda DEĞİŞMEYECEK.* Seed yalnız **eksik** anahtarları
  yazıyor (`DatabaseSeeder`), var olanı ezmiyor — bu bilinçli: operatörün elle girdiği değeri her
  deploy geri almamalı. Canlıda yeni değerler isteniyorsa süper admin Ayarlar ekranından girer.
  Sadece public form/müşteri giriş ekranını etkiler; uygulama içi marka koddan gelir.
- *Test talebi TEKSTIL-14 lokal demo veritabanında bırakıldı.* Canlıya gitmez, demo verisinin içinde.

- [x] **Faz 39 + 40 canlıya alındı** (`https://crm-kanban.runasp.net`, 2026-08-06 21:00).
  - **Paket biçimi düzeltildi:** `publish.ps1` **self-contained** üretiyor, ama canlı **framework-dependent**
    (`runtimeconfig.json`'da `frameworks` var, `coreclr.dll` yok). Ayrıca ilk denemede `-r win-x64`
    verildi ve `runtimes/` yerleşimi düzleşti → 30 dosya "değişti" göründü. RID'siz
    `--self-contained false` ile üretilince fark **26 dosyaya** indi; `web.config` ve
    `Microsoft.Data.SqlClient.dll` farkının **sahte** olduğu böyle anlaşıldı.
  - **`appsettings.Production.json` bilerek yüklenmedi** — canlının secret'larını taşıyor, repo
    sürümü onları silerdi. Yüklenen: **25 dosya, 1.03 MB zip**.
  - Yöntem (Faz 37 ile aynı): `app_offline.htm` (503 doğrulandı) → zip → panelde "Unzip"
    (`/wwwroot/`, üzerine yaz + app pool restart) → zip ve `app_offline` silindi.
  - **Canlı doğrulama:** kök 200, `<title>Kanby — Connect · Convert`, yeni paket
    (`index-qo95KB4U.js` 200, `index-Puy6it_c.css` 200), `kanby-logo.svg` + yeni `favicon.svg` 200,
    `POST /tickets/{id}/value` → **401** (404 değil, yani uç canlıda), süper admin girişi → pano
    yüklendi (**bu sorgu artık tutar sütunlarını okuyor → `TicketValue` migration'ı uygulanmış**),
    Kazanç ekranı `₺` ile açıldı ve **24 "tutarı girilmemiş talep"** saydı (yeni `finance.currency`
    seed'i de uygulanmış).
  - `publish-fd/` aynası deploy edilen 25 dosyayla güncellendi (bir sonraki farkın doğru çıkması için);
    `appsettings.Production.json` aynada da elle korundu.

**Açık iş (Faz 40 sonrası):**
- Sidebar marka adı ayardan okunmuyor (teknik borç #45).
- Canlıda `brand.system_name` hâlâ "CRM Kanban", `brand.logo_url` boş — seed var olan satırı ezmiyor
  (bilinçli). Public form/müşteri giriş ekranında eski ad görünür; süper admin Ayarlar'dan
  düzeltebilir. Uygulama içi marka koddan geldiği için etkilenmedi.
- Canlıda 6 eski asset dosyası (`index-CrgPuFhP.js*`, `index-CUG7wOMU.css*`) artık hiçbir yerden
  referans edilmiyor; zararsız, temizlenmedi.
- Canlıda `Seed:Demo` hâlâ `true` (Faz 34'ten devreden).

### Faz 41 — PDF rapor: müşteri mali durumu + harcanan süre ✅ (2026-08-06)

Kullanıcı isteği: "müşteri mali durum, ne kadar zamanda hangi müşteri ile uğraşılmış tarzı genel
istatistikleri PDF olarak export etmemiz lazım; dosya indirme özelliğini bu şekilde güncelleyelim."

- [x] **Yeni veri: `CustomerBreakdownItem`** — müşteri başına talep/açık/kazanılan/kaybedilen sayısı,
  kazanılan ve açık tutar, ortalama + toplam çözüm süresi, ortalama ilk yanıt, ilk/son talep tarihi.
- [x] **"Müşteri" tanımı = talebi açan, o talebin şirketinde **üyeliği olmayan** kişi** —
  `TicketAuthorizationService`'in erişim için kullandığı tanımın aynısı. Dışlama (şirket, açan)
  **çiftine** göre: aynı kişi bir şirkette personel, başkasında müşteri olabilir.
- [x] **PDF export (QuestPDF), CSV'nin yerine geçti.** A4 **yatay**; üç bölüm: genel istatistikler,
  mali durum, müşteri kırılımı tablosu + toplam satırı. Türkçe kültür (tr-TR) ve
  `system.timezone`'a göre yerel "oluşturulma" damgası.
- [x] **Aynı veri ekranda da var** (`/reports` → "Müşteri kırılımı" tablosu), böylece export
  indirmeden doğrulanabiliyor.
- [x] **8 yeni test** (`CustomerBreakdownTests` 7 + PDF smoke) — toplam **241 yeşil**.
- [x] **Lokal uçtan uca doğrulandı:** gerçek DB'den PDF üretildi, sayfa görüntü olarak incelendi
  (Türkçe karakterler, tablo hizası, toplam satırı).

**Karar/Varsayım (Faz 41):**
- *Kütüphane QuestPDF, PDFsharp/MigraDoc değil.* Belirleyici lisans değil **taşınabilirlik** oldu:
  projenin hem Windows IIS (canlı) hem Linux Docker (dev) yolu var; PDFsharp Linux'ta font resolver +
  gömülü font dosyası ister, QuestPDF kendi fontunu taşır ve iki ortamda aynı çıktıyı verir.
  **Lisans borcu:** QuestPDF Community yıllık brüt gelir 1M USD altında ücretsiz; üstünde satın
  alınmalı. Teknik borç #46 — bilinçli kabul edildi, gizlenmedi.
- *Lisans satırı `Program.cs`'te değil `ReportPdf` statik ctor'unda.* Startup yalnız API host'unda
  çalışıyor; startup'a yazınca üretimde çalışıp **testte patlıyordu**. Kütüphaneyi kullanan sınıfın
  yanında olması bu boşluğu kapatıyor — testin yakaladığı gerçek bir kusurdu.
- *CSV silindi, PDF'in yanında tutulmadı.* Eski export ham talep dökümüydü: atanan ve kategori
  sütunlarında **çıplak GUID**, para yok, müşteri yok, toplam yok — veritabanı yanında olmadan
  okunamazdı. Kullanıcı "dosya indirme özelliğini bu şekilde güncelleyelim" dedi. Excel girdisi
  gerçekten gerekirse ikinci bir format olarak bilinçle geri gelir, kalıntı olarak değil.
- *PDF, ekranın gördüğü **aynı** rapor nesnesinden üretiliyor.* Tek kod yolu → export ile pano
  çelişemez ve para kapısı iki kez değil bir kez uygulanır. Export, sızıntı için en doğal arka kapı.
- *Süre = takvim süresi, "çalışılan saat" değil.* Sistemde zaman kaydı (timesheet) yok. Ölçülmemiş
  bir sayıya "çalışılan saat" demek uydurma olurdu; hem PDF'te hem ekranda açıkça yazıyor.
- *Fiyatlanmamış talep toplama 0 olarak değil, **hiç** katılmıyor* — `RevenueSummary` ile aynı kural,
  böylece PDF'in iki bölümü birbiriyle çelişemiyor.
- *Para sütunları yetkisizde **tamamen kalkıyor**, boş bırakılmıyor.* Basılı bir tabloda boş para
  hücresi "sıfır" diye okunur; geliri sessizce eksik gösteren rapor, göstermeyenden kötüdür.
- *`Users` join'i LEFT.* Inner join, KVKK ile silinmiş bir kullanıcının taleplerini **genel
  toplamlardan da** düşürürdü — "toplam talep" bir kullanıcı kaydı yüzünden sessizce küçülürdü.
  Silinmiş açan "(silinmiş kullanıcı)" olarak görünür.
- *Müşteri toplamı satırı "yalnız müşteri talepleri" diye etiketlendi.* Bu toplam, "Mali durum"
  bölümünden meşru biçimde farklı (o, personel/yönetici taleplerini de sayar). Aynı etiketle iki
  farklı sayı, okuyanın rapora güvenmeyi bırakmasının yoludur.

**Faz 41'de bulunan ve kapatılan hata:**
- **Süper admin müşteri olarak listeleniyordu.** Süper adminin hiçbir yerde üyeliği yok (Faz 2'de rol
  bir *flag* olarak modellendi), dolayısıyla "üyeliği yok → müşteridir" kuralına takılıyordu.
  **İlk gerçek PDF çıktısına bakarken görüldü, testle değil** — yazılan testler kuralı doğruluyordu,
  kuralın kendisi eksikti. `IsSuperAdmin` dışlaması + regresyon testi eklendi.

- [x] **Faz 41 canlıya alındı** (2026-08-06 22:40) — **ve ilk denemede canlıda 500 verdi.**
  - Yükleme: `publish.ps1` self-contained ürettiği için yine RID'siz `--self-contained false` ile
    paketlendi. QuestPDF **her platformun** native Skia ikililerini taşıyor → ham fark 97 MB;
    canlı Windows olduğu için linux/osx/arm RID klasörleri yüklenmedi (42 dosya / 26 MB).
    Panelin yükleme aracı 10 MB'la sınırlı olduğundan zip **ikiye bölündü** (7.1 MB + 5.2 MB).
  - **PDF canlıda 500 verdi. Kök neden: MonsterASP app pool'u 32-bit (`.NET ... Core [x86]`).**
    Yönetilen kod AnyCPU olduğu için bugüne kadar hiç fark etmemişti; ama **native** bir kütüphane
    ilk kez devreye girdi ve win-x64 DLL 32-bit sürece yüklenemez. Ben de x86 RID klasörünü
    "gereksiz" diye elemiştim — tam da gereken oydu. `runtimes/win-x86/native` yüklendi, düzeldi.
  - **Teşhis yolu:** hata zarfı detay vermiyor (tasarım gereği). Canlıya dokunmadan önce
    **yayınlanmış paket lokalde çalıştırıldı → 200**. Yani sorun kodda değil host'ta; oradan
    panelde `.NET ... [x86]` görülüp neden bulundu. Canlıda log açma/kapama gerekmedi.
  - **Canlı doğrulama:** `GET /api/reports/global/export.pdf` → **200, `application/pdf`, 59.790 bayt,
    `%PDF-1.4`**; eski `export.csv` → 404 (kaldırıldığı doğrulandı).

**Açık iş (Faz 41 sonrası):**
- Panodaki "Personel yükü" hâlâ GUID'in ilk 8 hanesini gösteriyor (CSV'nin kapanan kusurunun ekran
  karşılığı) — teknik borç #47.
- Canlı 32-bit çalışıyor (teknik borç #48). Bundan sonra **her native bağımlılık** için x86 ikilisi
  şart; ayrıca 32-bit süreç ~2-4 GB adres alanıyla sınırlıdır. Panelden x64'e geçirilebilir ama
  çalışan siteyi etkileyeceği için bu turda yapılmadı.

### Faz 42 — SOLID/bileşen denetimi + tam kapsamlı seed ✅ (2026-08-07)

Kullanıcı denetim istedi: backend SOLID'e ne kadar uygun, UI yeterince bileşene bölünmüş mü,
gereksinim listesi karşılanıyor mu; ayrıca "durumlardan doğan hata mesajları bileşen içine yazılmış,
ortaklaştırılmalı" dedi ve tüm özellikleri kapsayan seed istedi.

**Denetim bulguları (ölçüm, tahmin değil):**
- Backend: en uzun iş metodu 67 satır (`InviteUserAsync`), servis ortalaması ~200 satır. 177 satırlık
  `OnModelCreating` bildirimsel EF yapılandırması (tek iş: modeli kur) — `IEntityTypeConfiguration`'a
  bölünebilir ama kiracı filtresinin **tek denetlenebilir yerde** kalması bilinçli tercih (borç #49).
- UI: 9 primitive + 4 alan bileşeni, ekran ortalaması ~136 satır. **Üç ölçülebilir tekrar bulundu ve
  kapatıldı** (aşağıda).
- Gereksinimler: RBAC (permission tablosu + kullanıcı-bazlı Grant/Deny), rol/yetki arayüzü, müşteri
  portalı + public form, e-posta bildirimi, S3, çok-şirketli admin, statü yönetimi, şirket/global
  rapor, ayarların DB'de + süper admin arayüzünde olması — hepsi kodda mevcut ve testli.

- [x] **Sorgu-durumu hata mesajları ortaklaştırıldı.** Faz 32 `catch` yolunu tek `errorText`'e
  indirmişti ama **`useQuery` hatalarını atlamıştı**: 10 ekranın her biri kendi cümlesini ve kendi
  markup'ını yazıyordu. Tek `LoadError` bileşeni + `loadErrorText()` kataloğu. **Asıl kazanç DRY
  değil doğruluk:** eski cümleler tahmindi ("Rapor yüklenemedi (yetki gerekebilir)") — sunucu zaten
  `report.forbidden` diye tam sebebi söylüyordu ve kimse bakmıyordu. Aynı kusur dosya yüklemede de
  vardı ("tip veya boyut" — sunucu beş ayrı sebebi ayırt ediyor), o da düzeltildi.
- [x] **`Select` ve `Textarea` primitive'i.** Aynı 60 karakterlik Tailwind dizisi 12 yerde
  kopyalanmıştı (biri farklı padding'e kaymış), `<textarea>` 7 yerde üç ayrı padding ile. Artık
  "bütün açılır listelerimiz aynı görünür" cümlesi yapısal olarak doğru, hatırlamaya bağlı değil.
- [x] **`ValueChanged` bildirim boşluğu kapatıldı.** Faz 40'ta olay tipi eklenmiş ama
  `NotificationMatrix`'e girilmemişti → tutar değişikliği **kimseye** bildirilmiyordu; sessizlik
  karar değil **ihmaldi**. Kural açıkça yazıldı: alıcı yalnız atanan personel, müşteri **asla** —
  tutar `ticket.value` ile korunuyor ama mail uygulamanın dışında okunur, orada hiçbir yetki
  filtreleyemez. Şablon olayı adlandırır, rakamı taşımaz. Güvenlik değişmezi olarak testlendi.
- [x] **Seed sistemin tamamını çalıştırır hale getirildi.**
  - **Talepler artık gerçek durum makinesinden geçiriliyor** (`MoveTo`), son statüde doğmuyor.
    Sebep ölçüldü: yalnız `ApplyStatus` `FirstResponseAt`/`ResolvedAt`/`ClosedAt` yazıyor; "Tamamlandı"
    doğan talepler bunların hiçbirini taşımıyordu, **bu yüzden raporda ve PDF'te bütün süreler "—"
    çıkıyordu**. Ayrıca durum makinesinden geçmeyen seed satırı, uygulamanın kendisinin reddedeceği
    bir durumu kodlayabilir.
  - Talep tutarları (kazanılan/kaybedilen/açık + bilerek fiyatsız 4 talep), tarihler ~5 aya yayıldı.
  - Yeni `SeedCrossCuttingAsync`: **aynı adminin ikinci şirketi** (şirket seçici artık görünüyor),
    **kullanıcı-bazlı 1 Grant + 1 Deny** (RBAC'i rol kümesinden ayıran şey), **1 güvenilir müşteri**
    (Faz 35 kuralı ancak biri güvenilir biri değilken görünür), **1 bildirim opt-out'u**.
  - **Seed artık host'u düşürmüyor.** Sıfırdan veritabanında seeder S3'e (MinIO) yazmaya çalışıp
    `Host terminated unexpectedly` ile uygulamayı komple öldürüyordu. Depo erişilemezse ek atlanır,
    seed devam eder; satır da yazılmaz (baytı olmayan ek indirmede 500 verirdi).
  - **Ölçülen sonuç:** 3 şirket, 11 kullanıcı, 20 talep (16 tutarlı, 4 kapanmış, 4 ilk-yanıtlı),
    20 yorum (4 iç not), 4 ek, 44 ticket olayı, 54 kuyruklanmış mail, 2 yetki override, 1 güven,
    1 bildirim tercihi. Rapor artık **41,0 sa ilk yanıt · 84,5 sa çözüm · %50 kazanma oranı ·
    %104,9 tahmin isabeti** üretiyor — hepsi eskiden "—" idi.
- [x] **242 test yeşil** (+1: değer bildirimi müşteriye gitmiyor).

**Karar/Varsayım (Faz 42):**
- *`LoadError` mesajı önce sunucu koduna bakar, `what` yalnız yedek.* Ekranın tahmin ettiği cümle
  yanlış olabilir; sunucunun söylediği sebep olamaz.
- *Para sütunu/`Select` gibi tekrarları bileşene çıkarmak "over-engineering" değil.* Ölçüt ikinci
  gerçek durum: 12 kopya, ikinci değil on ikinci durum.
- *Seed uygulama kurallarını taklit etmez, kullanır.* `MoveTo` gerçek `ChangeStatus`'ü çağırır;
  böylece demo verisi üretim verisiyle aynı değişmezlere uyar ve seed aynı zamanda durum makinesinin
  çalışan bir kanıtı olur.
- *Demo seed'i asla açılışı bloke etmemeli.* Depo bir ağ bağımlılığı; onun yokluğu bir kolaylık
  özelliğini kapatır, uygulamayı değil.

**Canlıya alma — ve canlıyı düşüren bir olay (2026-08-07):**
- İlk deploy (19 dosya) sonrası canlı **HTTP 500.30** verdi: uygulama açılışta çöktü, site ~20 dakika
  kapalı kaldı. Sebep panelin **Event log**'undaki yakalanmış stdout'ta bulundu:
  `Cannot insert duplicate key row in object 'dbo.CustomerTrusts' with unique index IX_CustomerTrusts...`
- **Kök neden benim yeni `SeedCrossCuttingAsync`'im idempotent değildi.** Slug guard'ı yalnız "bu blok
  daha önce çalıştı mı" sorusunu cevaplıyor; satırların **başka bir yoldan** gelmiş olabileceğini değil.
  Canlıda bir operatör o müşteriyi zaten moderasyon ekranından "Onayla + güven" ile onaylamıştı, yani
  `CustomerTrusts` satırı vardı. Düz `Add` unique index'e çarptı ve **açılışı öldürdü**.
- **İki ayrı düzeltme yapıldı, ikisi de gerekliydi:**
  1. *Demo seed artık açılışı bloke edemez.* `Program.cs`'te `DevSeeder` çağrısı try/catch içine alındı
     ve hata `Log.Error` ile geçiliyor. Şema (migrate) ve gerçek seed (yetkiler, statüler, süper admin)
     başarısız olursa uygulama **açılmamalı** — ama demo verisi bir kolaylıktır, site yine açılmalı.
  2. *Seed ekleri insert-if-missing yapıldı* (yetki override, güven, bildirim tercihi) — projenin geri
     kalanının zaten uyduğu kural.
- Ayrıca teşhis sırasında **ikinci bir gerçek hata** bulunup düzeltildi: `SingleAsync` +
  `IgnoreQueryFilters` (soft-delete edilmiş şirket varsa iki satır döner ve patlar) — **teknik borç
  #42'nin tam olarak uyardığı tuzak**, dördüncü kez. Olayın sebebi bu değildi ama bekleyen bir bombaydı.
- Canlı doğrulama: kök 200, `export.pdf` 401 (uç yerinde), giriş → pano yüklendi,
  `GET /companies` → **"Anadolu Tekstil İhracat"** listede, yani ek seed canlıda uygulandı.
...
| 43 | **CSV export'ta tutar sütunu yok** (Faz 39). Rapor JSON'ı `ticket.value`'ya göre para alanını çıkarıyor, CSV ise tutarı hiç taşımıyor — sızıntı değil, eksik özellik. Eklenirse sütun yetkisizden **tamamen** çıkarılmalı (boş bırakılmamalı: boş hücre "0" diye okunur). | Düşük |
| 50 | **Seed'ler açılışı bloke edebiliyordu — kapatıldı ama ders kalıcı** (Faz 42): demo seed artık try/catch içinde. Aynı soru gerçek seed (`DatabaseSeeder`) için de sorulmalı: o başarısız olursa uygulama açılmamalı (doğru davranış), ama hata mesajı log'da net görünmeli. Bir sonraki dokunuşta gözden geçir. | Düşük |
| 49 | **`CrmDbContext.OnModelCreating` 177 satır** (Faz 42 denetimi). Tek iş yapıyor (modeli yapılandır) ve kiracı filtresinin tek denetlenebilir yerde olması bilinçli; yine de `IEntityTypeConfiguration<T>` sınıflarına bölünebilir. Bölünürse **filtreler dağılmasın** — sızıntı riskini artırır. | Düşük |
| 48 | **Canlı app pool 32-bit (`[x86]`)** — Faz 41'de PDF'in canlıda 500 vermesinin kök nedeni buydu (win-x64 native DLL 32-bit sürece yüklenemez). Bundan sonra eklenen **her native bağımlılığın** `runtimes/win-x86/native` ikilisi de yüklenmeli. Ayrıca 32-bit süreç adres alanı sınırlıdır (~2-4 GB). Panelden x64'e geçirilebilir; geçilirse x86 klasörleri gereksizleşir. | Orta |
| 46 | **QuestPDF Community lisansı gelir eşiğine bağlı** (Faz 41): yıllık brüt gelir 1M USD altında ücretsiz, üstünde ticari lisans satın alınmalı. Bildirim `ReportPdf` statik ctor'unda tek satır. Eşiğe yaklaşılırsa ya lisans alınır ya MIT bir kütüphaneye (PDFsharp/MigraDoc) geçilir — o durumda Linux için font resolver + gömülü font gerekir. | Orta |
| 47 | **Panoda "Personel yükü" atanan kişiyi GUID'in ilk 8 hanesiyle gösteriyor** (`Dashboard.tsx`). Faz 41'de silinen CSV'nin aynı kusuru; rapor DTO'su `assignedToId` taşıyor, isim taşımıyor. Personel adlarını `useMembers` ile eşle ya da DTO'ya isim ekle. | Düşük |
| 45 | **Sidebar/giriş ekranındaki marka adı kodda sabit** ("Kanby"), `brand.system_name` ayarından okunmuyor (Faz 40). Ayardan okumak `/settings` ucunun personele açılmasını gerektirir — bu bir yetki genişlemesi. Gerekirse marka alanlarını dönen ayrı bir `GET /public/brand` ucu doğru çözüm. | Düşük |
| 44 | **Kazanç/tutar sekmesi navigasyonda yetkiye göre gizlenmiyor** — `ticket.value` olmayan personel `/revenue`'yu görüyor ve "yetkiniz yok" kartıyla karşılaşıyor. Sunucu zaten veriyi vermiyor (güvenlik sorunu değil), ama nav tüm staff sekmeleri gibi statik; menüyü yetkiye göre süzmek ayrı bir iş (Şirketler/Yetkiler sekmeleri de aynı durumda). | Düşük |
